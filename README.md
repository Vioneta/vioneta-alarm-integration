# Vioneta Alarm Integration
[![vps_badge](https://img.shields.io/badge/VPS-Default-orange.svg)](https://github.com/Vioneta/integration)

This is an alarm system integration for Vioneta Platform. It provides a user interface for setting up your own alarm system completely from the browser.

- [Vioneta Alarm Integration](#vioneta-alarm-integration)
...
## Introduction
This is an integration for the `alarm_control_panel` domain in Vioneta.
It allows to combine existing sensors for creating a security system for your house. 
The integration is comparable with the [Manual Alarm](https://www.vioneta.com/integrations/manual/) in Vioneta, but also has some additional features that makes it easier to use. 

Alarmo consists of 3 parts:
* Alarmo component: a custom component for Vioneta that controls the states of the alarm panel entities in Vioneta.
...
### Features
* Fully compatible with Vioneta and the [Alarm Panel Card](https://www.vioneta.com/lovelace/alarm-panel/).
...
* Supports configuring your existing Vioneta entities as security sensors. These sensors will be watched automatically. 
* Allows setting up multiple users with individual pincode and permission levels.
* Will restore its previous state after restart of Vioneta.
...
![](https://raw.githubusercontent.com/Vioneta/vioneta-alarm-integration/main/screenshots/Preview.png)
...
<li><u>Manually:</u><br>
Download the <a href="https://github.com/Vioneta/vioneta-alarm-integration/releases">latest release</a> as a zip file and extract it into the `custom_components` folder in your Vioneta installation.</li>
</ul>
<li>Restart Vioneta to load the integration into Vioneta.</li>
...
<li><u>Manually:</u><br>
Download the <a href="https://github.com/Vioneta/vioneta-alarm-integration/releases">latest release</a> as a zip file and extract it into the <code>custom_components</code> folder in your Vioneta installation, overwriting the previous installation.
</li>
</ul>
<li>Restart Vioneta to load the changes.</li>
<li>(Optional) Verify the version number.
<ul>
<li><u>Verify version of the backend</u>:<br>
In Vioneta, go to Configuration -> Integrations. In the Alarmo card, you should see a link with '1 device', click it. In the table click the 'Alarmo' device, and you should see the Device info. The 'firmware version' represents the installed version number.
</li>
...
<ol>
<li><u>Remove Alarmo from Vioneta:</u><br>
In Vioneta go to Configuration -> Integrations. In the Alarmo card, click the button with the 3 dots, and click 'Delete'.
</li>
<li>Remove the files:
<ul>
<li>
...
<li>Restart Vioneta to make all traces of the component dissapear.
</li>
</ol>
</details>
...
![](https://raw.githubusercontent.com/Vioneta/vioneta-alarm-integration/main/screenshots/states.png)
...
After installing Alarmo the entity `alarm_control_panel.alarmo` shall be added to Vioneta.
...
#### States
The Alarmo entity follows the state definitions as defined by Vioneta:
...
Currently Alarmo supports sensors of type `binary_sensor`. Alarmo will check the [device class](https://www.vioneta.com/integrations/binary_sensor/#device-class) of each sensor, and only supports sensors that are related to security.
...
Vioneta defines [device classes](https://www.vioneta.com/integrations/binary_sensor/#device-class) for binary sensors.
...
By default, the alarm has no code and can be locked and unlocked by anyone who has access to Vioneta.
...
It is recommended to use the same name as your Vioneta account, but this is not required.
...
Your code is stored completely secure. It is encrypted in the same way as your login credentials, and stored in the Vioneta storage registry. When you enter a code, this will be encrypted too, and the encrypted values will be compared for a match.
...
The command topic is an input topic, i.e. the topic is watched by Alarmo and data should be sent by another application (such as a wall panel). The data should never be sent with *retain* flag, as this might give undesired behaviour when Vioneta is restarted.
...
* `<area_name>` is a *slug* of the name that is given to an area. This means that the name shall be in lowercase and all non-alphanumerical characters are replaced by underscores (similar to the entity_IDs in Vioneta).
...
For using this, you first need to install the Vioneta app on your iOS or Android device.
...
<img src="https://raw.githubusercontent.com/Vioneta/vioneta-alarm-integration/main/screenshots/notification_gui.png">
...
For more info about actionable notifications in Vioneta, see [here](https://companion.home-assistant.io/docs/notifications/actionable-notifications/).
...
3. Choose an event for which you would like to trigger a device, pick the Vioneta entity from the list, choose whether it should be switched on, and save the automation.
4. Now the device should automatically be triggered together with the alarm :tada:

Currently the following Vioneta entity types are supported: `switch`, `input_boolean`, `light`, `script`.

The actions are currently limited to *turning on* or *turning off* a Vioneta entity. 


##### Advanced actions
In case the built-in Actions panel does not offer the flexibility you are looking for, you can set up more flexible automations in Vioneta.
...
#### Automatic arming
If you want to control the state of Alarmo through an external device (e.g. a keyfob, button panel, or phone with geofencing), you can do so by means of a Vioneta automation.

This kind of automation cannot be made from the Alarmo "Actions" panel, you should use a Vioneta automation instead (Configuration -> Automations).

Example of Vioneta automation for arming with a remote button:
```yaml
trigger:
  - platform: state
...
For controlling the alarm through the Vioneta frontend, you can set up a Lovelace card.
There are two cards available:
* [Lovelace Alarm Panel card](https://www.vioneta.com/lovelace/alarm-panel/) which comes with Vioneta. This offers basic arm/disarm functionality and displays the current state.
* [Alarmo-card](https://github.com/Vioneta/vioneta-alarm-integration-card) which is intended as a companion for Alarmo. It offers some extra functionality on top of the standard alarm panel card, such as countdown timer for exit / entry delay and feedback messages with sensor(s) causing the alarm to be triggered or unable to arm.


---

## Third party platforms
This section is intended to give a starting point for users who want to operate Alarmo outside of Vioneta.
...
Follow the instructions on the [Vioneta page](https://www.vioneta.com/integrations/homekit/) to establish a gateway between Vioneta and Homekit.

Alarmo entities are of type `alarm_control_panel`, so make sure to configure the homekit gateway in Vioneta have these included. 
...
* Alarmo allows enabling/disabling arm modes 'on the fly', however it is needed to restart Vioneta (or reconfigure Homekit via Vioneta) before this becomes effective in Homekit.
...
The first step is to set up the [Google Assistant](https://www.vioneta.com/integrations/google_assistant/) integration in Vioneta.
For Nabu Casa subscribers, this requires just a few clicks, other users need to register a project in Google cloud first (see instructions in Vioneta docs).
...
* The Google Translate [text-to-speech service](https://www.vioneta.com/integrations/google_translate/) can be very useful to provide additional feedback (e.g. when arming fails) for the members . You can set up a notification action via Alarmo.
